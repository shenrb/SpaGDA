...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        # basic parameters
        parser.add_argument('--name', type=str, default='nsclc', help='name of the experiment. It decides where to store samples and models')
        parser.add_argument('--gpu_ids', type=str, default='5', help='gpu ids: e.g. 0  0,1,2, 0,2. use -1 for CPU')
        parser.add_argument('--checkpoints_dir', type=str, default='./checkpoints', help='models are saved here')

        # Model Generator parameters
        parser.add_argument('--input_dim', type=int, default=929, help='Dim of input, using HVG')
        parser.add_argument('--feat_hidden1', type=int, default=400, help='Dim of G-encoder hidden layer 1.')
        parser.add_argument('--feat_hidden2', type=int, default=160, help='Dim of G-encoder hidden layer 2.')
        parser.add_argument('--gcn_hidden1', type=int, default=80, help='Dim of G-VGAE hidden layer 1.')
        parser.add_argument('--gcn_hidden2', type=int, default=40, help='Dim of G-VGAE hidden layer 2.')
        parser.add_argument('--p_drop', type=float, default=0.2, help='Dropout rate.')
        parser.add_argument('--sc_neighbors', type=int, default=10, help='K nearest neighbors to create sc graph.')
        parser.add_argument('--st_neighbors', type=int, default=15, help='K nearest neighbors to create st graph.')
        parser.add_argument('--dis_sigma', type=int, default=150, help='dis sigma of  nearest neighbors to weight st graph.')
    ...
    def print_options(self, opt):
        ...
        file_name = os.path.join(expr_dir, '%s_opt_%s.txt'%(opt.phase, time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())))
...
class TrainOptions(BaseOptions):
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--sc_data', type=str, default='data/scRNA_cc.h5ad', help='source scRNA-seq dataset, h5ad format.')
        parser.add_argument('--st_data', type=str, default='data/nsclc_cc.h5ad', help='target spatial transcriptomic dataset, h5ad format.')

        # Discriminator parameters
        parser.add_argument('--dis_model', type=str, default='conv', help='discriminator model [mlp | conv]')
        parser.add_argument('--ndf', type=int, default=4, help='# of discriminator (D, netD) filters in the first conv layer.')
        parser.add_argument('--n_layers_D', type=int, default=1, help='netD n_layers')
        parser.add_argument('--norm', type=str, default='instance', help='instance normalization or batch normalization [instance | batch | none]')
        parser.add_argument('--cls_loss', type=str, default='fl', help='discriminator model [ce | fl]')

        # Training parameters
        parser.add_argument('--phase', type=str, default='train', help='train, val, test, etc')
        parser.add_argument('--classes', type=int, default=16, help='number of classes for CLS branch')
        ...
        parser.add_argument('--cells_per_batch', type=int, default=320, help='random sampling #number of cells per epoch')
