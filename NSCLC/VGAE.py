...
class VGAEModel(nn.Module):
    def __init__(self, opt):
        ...
        #self.decoder.add_module('decoder_L0', full_block(self.latent_dim, opt.feat_hidden1, opt.p_drop))
        self.decoder.add_module('decoder_L1', full_block(self.latent_dim, opt.input_dim, opt.p_drop))

        # GCN layers
        self.gc1 = GraphConvolution(opt.feat_hidden2, opt.gcn_hidden1, opt.isTrain, opt.p_drop, act=lambda x: x)  # F.relu
